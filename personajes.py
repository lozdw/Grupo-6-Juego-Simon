...
class Personaje(ABC):
    ...
    def reiniciar_habilidad(self, estado_juego=None):
        ...
        if estado_juego is not None:
            aliases = (
                ...
                "shield",
                "proteccion",
                "bloquear_penalizacion",
                "primer_error_protegido",
                "primer_error",
                "error_actual",
            )
            for alias in aliases:
                try:
                    setattr(estado_juego, alias, False)
                except Exception:
                    pass
    ...
    def _activar_escudo(self, estado_juego):
        ...
        for alias in aliases:
            try:
                setattr(estado_juego, alias, True)
            except Exception:
                pass

    def _sumar_tiempo(self, estado_juego, valor):
        if hasattr(estado_juego, "t_total"):
            estado_juego.t_total = getattr(estado_juego, "t_total") + valor
        if hasattr(estado_juego, "t_restante"):
            estado_juego.t_restante = getattr(estado_juego, "t_restante") + valor
...
class Miku(Personaje):
    ...
    def _aplicar_habilidad(self, estado_juego):
        self._sumar_tiempo(estado_juego, 5.0)
...
class Neru(Personaje):
    ...
    def _aplicar_habilidad(self, estado_juego):
        ...
        if hasattr(estado_juego, "error_actual"):
            estado_juego.error_actual = False
        if hasattr(estado_juego, "primer_error"):
            estado_juego.primer_error = False
        if hasattr(estado_juego, "bloquear_penalizacion"):
            estado_juego.bloquear_penalizacion = True
        try:
            setattr(estado_juego, "primer_error_protegido", True)
        except Exception:
            pass
